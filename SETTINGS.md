# Roses Settings Reference

All settings are stored in `settings.json` in the app config directory (`%APPDATA%/Roses/settings.json`). The file is a flat JSON object with `bloom-` prefixed keys. Roses watches this file for external changes and applies them in real-time.

## Quick Start

Edit `settings.json` with any text editor while Roses is running. Changes are applied immediately — no restart required.
...
```json
{
	...
	"bloom-scale": "1.0",
	"bloom-target-monitor": "1",
	"bloom-high-priority-startup": "true"
}
```
...
| ----------------------------- | -------------------- | --------- | ------------------------------------------------------------------------------------------------------------- |
| `bloom-target-monitor`        | string (`""`/`"0"`/`"1"`) | `""` | Target monitor index for Roses windows and hooks. `""` = Primary display, `"0"` = Display 1, `"1"` = Display 2. |
| `bloom-high-priority-startup` | `"true"` / `"false"` | `"false"` | Register Roses in Windows Task Scheduler with highest runlevel and zero delay to run before other startup apps. |

### Dock

| Key                             | Type                             | Default   | Description                                                                                                                                                                      |
| ------------------------------- | -------------------------------- | --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `bloom-dock-enabled`            | `"true"` / `"false"`             | `"true"`  | Show or hide the Roses Dock (taskbar replacement).                                                                                                                               |
...
| `bloom-media-visualizer-enabled`    | `"true"` / `"false"`           | `"true"`      | Audio visualizer bars in music mode.                                                            |
...
| Key                                  | Type                     | Default    | Description                                                              |
| ------------------------------------ | ------------------------ | ---------- | ------------------------------------------------------------------------ |
| `bloom-volume-overlay-enabled`       | `"true"` / `"false"`     | `"true"`   | Show Roses volume HUD when volume changes (replaces native Windows OSD). |
| `bloom-volume-edge-enabled`          | `"true"` / `"false"`     | `"true"`   | Show volume edge glow bar on screen edge.                                |
| `bloom-brightness-overlay-enabled`   | `"true"` / `"false"`     | `"true"`   | Show Roses brightness HUD when brightness changes.                       |
| `bloom-brightness-edge-enabled`      | `"true"` / `"false"`     | `"true"`   | Show brightness edge glow bar on screen edge.                            |

### Appearance

| Key                      | Type                                             | Default     | Description                                                                                                                 |
| ------------------------ | ------------------------------------------------ | ----------- | --------------------------------------------------------------------------------------------------------------------------- |
| `bloom-theme-mode`       | `"dark"` / `"light"` / `"custom"` / `"adaptive"` | `"dark"`    | Theme mode. **dark** = dark translucent. **light** = light translucent. **custom** = user-picked color. **adaptive** = accent. |
...
Roses uses two Tauri events for settings synchronization:

### `settings-changed`

- **Emitted by:** `save_setting` command (frontend or backend)
- **Payload:** `{ "key": "bloom-...", "value": ... }`
- **Purpose:** Broadcasts changes made through the Roses UI to all windows
...
## Example: Changing Settings via Script

```powershell
# PowerShell: switch dock to smart mode
$json = Get-Content "$env:APPDATA\Roses\settings.json" | ConvertFrom-Json
$json.'bloom-dock-mode' = 'smart'
$json | ConvertTo-Json | Set-Content "$env:APPDATA\Roses\settings.json"
```
...
- Changing `bloom-scale` or `bloom-target-monitor` triggers AppBar and window repositioning to adjust the reserved screen space on the chosen monitor.
